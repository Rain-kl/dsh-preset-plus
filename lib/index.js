...
function installInjector(ctx, cfg) {
  ...
  // 手动 /prefill 标记的会话：即使 autoMode 关闭也强制注入一次。
  const force = new Set();

  const readPreset = () => {
    try {
      return core.loadPreset();
    } catch {
      return null;
    }
  };
  const readEntries = () => readPreset()?.entries ?? [];
  ...
  ctx.on("llm/stream", (options, next) => {
    ...
    // 自动注入是否开启（来自预设数据 preset.autoMode）；手动 /prefill 可强制。
    ...
    // 标记注入（一次会话仅一次）。手动 /prefill 在注入后清除 force 标记。
      ...
      host.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            res.writeHead(200, {
              "content-type": "application/json; charset=utf-8",
              "content-disposition": `attachment; filename="preset-plus-${encodeURIComponent(preset.name || "preset")}.json"`,
            });
            res.end(raw);
          } catch (e) {
            json(res, 500, { ok: false, error: String(e) });
      ...
      host.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            const body = await readBody(req);
            let parsed;
            // 允许 body.raw 为字符串（JSON 文本）或 body.preset 为对象
            if (typeof body?.raw === "string") {
              parsed = JSON.parse(body.raw);
            } else if (body?.preset && typeof body.preset === "object") {
              parsed = body.preset;
            } else {
              json(res, 400, { ok: false, error: "需要 raw(JSON 文本) 或 preset(对象)" });
              return;
            }
            // 导入前校验版本：不一致则阻止导入（不覆盖保存）。
            core.assertImportVersion(parsed);
            const saved = await core.savePreset(parsed);
            json(res, 200, { ok: true, preset: saved });
          } catch (e) {
  ...
  if (commands) {
    commands.register({
      ...
      handler: async (invocation) => {
        ...
        // CommandInvocation 携带的是 agent（Agent），不是 sessionId；用
        // agent.id（即 SessionId）作会话标识，与自动注入的 options.sessionId 一致。
