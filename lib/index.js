...
//        - system 条目 → 追加到现有 options.system（不覆盖其他插件提示词）
//        - 随后的 user / assistant 条目 → 按顺序插入 messages 前端，
//          构造「伪装模型输出」预填充上下文，让真实模型在已配合的语境下输出。
//        - 每个新的主请求（purpose 为空）都注入一次；同一请求内部重入跳过。
//          fake 消息不写入会话历史，所以必须在每次发给模型前重新前置。
//   4. AB 双模式：auto（自动，可关）+ /prefill（手动强制，即使 autoMode 关闭）。
//   5. /preset-plus 命令 + 模型工具 + HTTP API（设置页）。
//
// 注入定位：llm/stream 瀑布以副本方式追加 system/messages，保留其他插件贡献；
...
function installInjector(ctx, cfg) {
  ...
  const force = new Set();
  // Track only fake messages created by this plugin so /prefill can replace
  // them without removing messages contributed by other plugins.
  const injectedMessageIds = new Map();
  // 同一条请求内部重入（return ctx.llm.stream(mutableOptions)）时跳过，避免无限循环。
  // 每个新的主请求都是新的 options 对象，不会命中这个集合。
  ...
  ctx.on("llm/stream", (options, next) => {
    ...
    console.log("[preset-plus] injected → session=" + sessionId +
      ", inject=" + inject.length + ", system=" + (system ? "追加" : "保留"));
    ...
    const mergedSystem = system === undefined
      ? options.system
      : [options.system, system].filter(Boolean).join("\n\n");
    const mutableOptions = {
      ...options,
      ...(inject.length > 0 ? { messages: [...inject, ...sourceMessages] } : { messages: sourceMessages }),
      ...(mergedSystem !== undefined ? { system: mergedSystem } : {}),
    };
...
//
//  让用户能用"一行命令"安装：插件激活时（DSH 启动），若
//  <dshHome>/.agent-presets/preset-plus/ 不存在，就从随包发布的 preset/
//  目录复制过去（preset.yml 里的 PresetPlus 名称/描述 + agent.cordis.yml）。
//  已存在则保留，不覆盖用户编辑。这样无需任何 npm 生命周期脚本
//  （prepare/postinstall 会被 pnpm 10 默认拦截，破坏一行安装）。
// ═══════════════════════════════════════════════════════════════════
...
/** 复制随包 preset/ 目录到 DSH 模式目录（幂等；目标已存在则跳过）。 */
async function ensureAgentPresetMode() {
  const target = agentPresetTargetDir();
  try {
    await fsp.mkdir(dirname(target), { recursive: true });
    const marker = join(target, "agent.cordis.yml");
    const markerExists = fs.existsSync(marker);
    const shouldCopy = !markerExists || !fs.existsSync(join(target, "preset.yml"));
    if (!shouldCopy) {
      console.log(`[preset-plus] initialized: ${target}`);
      return;
    }
    // 复制 preset/ 目录（含 preset.yml 与 agent.cordis.yml）。
    await fsp.rm(target, { recursive: true, force: true });
    await fsp.mkdir(target, { recursive: true });
    await fsp.cp(PRESET_DIR, target, { recursive: true });
    console.log(`[preset-plus] created ${PRESET_MODE_ID}: ${target}`);
  } catch (e) {
    // 模式创建失败不应阻断插件：记录即可，注入仍按 scopedPresets 门禁工作。
