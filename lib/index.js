...
async function ensureAgentPresetMode() {
  ...
  try {
    ...
    if (!shouldCopy) {
      console.log(`[preset-plus] 模式已存在，保留现有配置: ${target}`);
      return;
    }
    ...
    console.log(`[preset-plus] 已创建模式 ${PRESET_MODE_ID}: ${target}`);
  } catch (e) {
    // 模式创建失败不应阻断插件：记录即可，注入仍按 scopedPresets 门禁工作。
    console.warn(`[preset-plus] 创建模式失败: ${e instanceof Error ? e.message : String(e)}`);
  }
}
