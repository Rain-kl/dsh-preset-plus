...
/** 构造要注入的消息序列。返回 { system, prepend }：
 *  - system: 覆盖 options.system 的文本（若存在 system 条目）
 *  - prepend: 要插到 messages 前端的 Message[]（fake user + fake assistant）
 */
function buildInjection(entries, makeUserMessage, makeAssistantMessage, provider, model) {
  ...
  for (const e of entries) {
    ...
    if (e.role === "user") {
      prepend.push(makeUserMessage({ text: e.text }));
    } else if (e.role === "assistant") {
      prepend.push(makeAssistantMessage({ text: e.text, provider, model }));
    }
  }
...
function installInjector(ctx, cfg) {
  ...
  ctx.on("llm/stream", (options, next) => {
    ...
    // ── 临时诊断（定位根因，定位后移除）──────────────────────────────
    // 打印每个请求的关键值，判断"监听器是否触发"与"作用域判据是否命中"。
    let diagComposed;
    let diagAgent;
    try {
      diagAgent = ctx.agents?.get(sessionId);
      diagComposed = agentPresets ? agentPresets.composedPreset(diagAgent?.ctx) : "(no agentPresets)";
    } catch (e) {
      diagComposed = "(composedPreset threw: " + String(e) + ")";
    }
    console.log("[preset-plus][diag] llm/stream fired:",
      "sessionId=" + sessionId,
      "| provider=" + options.provider,
      "| model=" + options.model,
      "| msgCount=" + (options.messages?.length ?? 0),
      "| purpose=" + String(options.purpose),
      "| isMain=" + isMain,
      "| agent=" + (diagAgent ? diagAgent.id : "(undefined)"),
      "| composedPreset=" + JSON.stringify(diagComposed),
      "| scoped=" + JSON.stringify(cfg.scopedPresets),
      "| agentPresets=" + (agentPresets ? "yes" : "no"),
    ...
    console.log("[preset-plus][diag2] shouldInject=" + shouldInject,
      "| isMain=" + isMain,
      "| enabledForAgent=" + enabledForAgent(agentPresets, ctx.agents, sessionId, cfg),
      "| injected=" + injected.has(sessionId),
      "| options.messages.len=" + (options.messages?.length ?? 0));
    if (!shouldInject) return next();

    const entries = readEntries();
    console.log("[preset-plus][diag2] entries.len=" + entries.length,
      "| ctx.get('message')=" + (ctx.get("message") ? typeof ctx.get("message") : "(none)"));
    if (entries.length === 0) return next();

    const { system, prepend } = buildInjection(
      entries,
    ...
    console.log("[preset-plus][diag2] prepend.len=" + prepend.length,
      "| system.len=" + (system ? system.length : 0),
    ...
    const mutableOptions = {
      ...options,
      ...(prepend.length > 0 ? { messages: [...prepend, ...(options.messages || [])] } : {}),
...
/** 创建 user 消息。始终用内置 messageFactory（可靠、无 peer 依赖）。 */
...
// 一个与源码一致的轻量 message 工厂（无 dsh-llm peer 依赖）：
// 与 @deepseek-ai/dsh-llm message.ts 的 createMessage 一致——deepFreeze + randomUUID。
